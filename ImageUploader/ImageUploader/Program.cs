...
namespace ImageUploader
{
    class Program
    {
        ...
        private static void DoWork(int count, int numOfBatches)
        {
            ...
            {
                tasks.Add(Task.Factory.StartNew(() => UploadImages(count)));
            }
        ...
        private static int UploadImages(int batchSize)
        {
            int reconcilationId;
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    if (int.Parse(blobExists) == uniqueId)
                    {
                        byte[] image = HttpGet(cloudinaryURL);

                        result = UploadToBlobStorage(image, container_id, file_name);
                        ...
                        WriteToConsole($"Thread {Thread.CurrentThread.ManagedThreadId}: validation status result: {result}", ConsoleColor.White);
        ...
        private static int GetTotalRecords()
        {
            ...
            sqlCmd.CommandText = $@"SELECT COUNT(1) FROM [reconcilation]
                                   WHERE ([cloudinaryExists] = 1 and [blobExists] = 0)";
        ...
        private static bool UploadToBlobStorage(byte[] image, string container_id, string file_name)
        {
            try
            {
                CloudStorageAccount storageAccount = CloudStorageAccount.Parse(CloudConfigurationManager.GetSetting("StorageConnectionString"));
                ...
                CloudBlobContainer container = blobClient.GetContainerReference(container_id);
